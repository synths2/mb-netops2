...
import meraki_store
from settings import DATASTORE_MAX_DAYS as timeSinceMax
from settings import CLIENT_TIMESPAN as clientTimeSpan
...
def __hasorgaccess(apikey, targetorg):
    ...
    dashboard = urlfetch.fetch(geturl, headers=headers)
    currentorgs = json.loads(dashboard.content)
    orgs = []
    validjson = __isjson(dashboard.content)
...
def __returnhandler(statuscode, returntext, objtype, suppressprint=True):
    """
    ...
    if str(statuscode) == '200' and validreturn:
        if suppressprint is False:
            logging.info('{0} Operation Successful - See returned data for results\n'.format(str(objtype)))
            #pass
        return returntext
...
def getorgadmins(apikey, orgid, suppressprint=False):
    ...
    #
    result = __returnhandler(dashboard.status_code, dashboard.content, calltype, suppressprint)
    return result
...
def getclientslist_stub(apikey, serialnum, timestamp=86400, suppressprint=True, networkName=None, networkId=None):
    calltype = 'Device Clients getclientslist_stub'
    ...
    dashboard = urlfetch.fetch(geturl, headers=headers)
    result = __returnhandler(dashboard.status_code, dashboard.content, calltype, suppressprint=suppressprint)

    storeClientList = meraki_store.get_meraki_client_list(serialnum)
    clientList = []
    #logging.debug("storeClientList clientList is {}".format(storeClientList.clientList))

    if result:
        for client in result:
            clientList.append(client['id'])

            storeClient = meraki_store.get_meraki_client(client['id'])
            #logging.debug("storeClient is {}".format(storeClient.mac))
            if not storeClient.ip == client['ip']:
                #logging.debug("didn't find stored client, storing a new one")
                logging.debug("ip of existing client {} didn't match {}".format(storeClient.ip, client['ip']))
                storeClient.description = client['description']
                storeClient.dhcpHostname = client['dhcpHostname']
                storeClient.ip = client['ip']
                storeClient.client_id = client['id']
                storeClient.mac = client['mac']
                storeClient.mdnsName = client['mdnsName']
                storeClient.switchport = client['switchport']
                storeClient.usage_sent = str(client['usage']['sent'])
                storeClient.usage_recv = str(client['usage']['recv'])
                storeClient.networkName = networkName
                storeClient.networkId = networkId
                storeClient.put_async()
            else:
                logging.debug("client {} already had correct ip {}".format(storeClient.ip, client['ip']))
        storeClientList.clientList = clientList
        storeClientList.put()
    else:
        logging.debug("no clients found for device {}".format(serialnum))
        logging.debug(storeClientList.timestamp)
        return storeClientList
    return storeClientList


def getclients(apikey, serialnum, timestamp=86400, suppressprint=False, force=False, networkId=None, networkName=None):

    storeClientList = meraki_store.get_meraki_client_list(serialnum)
    #storeDevice = meraki_store.get_meraki_device(serialnum)

    if storeClientList.clientList:

        try:
            dayssince = (datetime.datetime.now() - storeClientList.timestamp).days
            msg = 'time since last update is {} days.'.format(str(dayssince))
            logging.debug(msg)

            if dayssince < timeSinceMax and force == False:
                logging.info("clients were refreshed less than a day ago, skipping")
                return storeClientList
            else:
                logging.info("clients were retrieved over a day ago, storing again")
                clientList = getclientslist_stub(apikey, serialnum, timestamp,
                                                 suppressprint=False, networkId=networkId, networkName=networkName)
                #logging.debug(clientList.clientList)
                return clientList

        except:
            logging.debug("in except block, couldn't find storeClientList timestamp value")
            #logging.debug(storeClientList.clientList)
            return storeClientList

    else:
        logging.info("no existing client list found, retrieving details...")

        clientList = getclientslist_stub(apikey, serialnum, timestamp, suppressprint=False, networkId=networkId,
...
def updateclientpolicy(apikey, networkid, clientmac, policy, policyid=None, suppressprint=False):
    calltype = 'Device Clients'
    puturl = '{0}/networks/{1}/clients/{2}/policy?timespan=86400'.format(str(base_url), str(networkid),
                                                                           str(clientmac))
    #puturl = 'https://n177.meraki.com/api/v0/networks/{0}/clients/{1}/policy?timespan=86400'.format(str(networkid),
    #                                                                                              str(clientmac))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    ...
    if policy not in ('whitelisted', 'blocked', 'normal', 'group'):
        logging.debug('Parameter policy must be either whitelisted, blocked, normal, or group with ID specified')
    ...
    dashboard = urlfetch.fetch(url=puturl, method=urlfetch.PUT, payload=json.dumps(putdata), headers=headers,
                           follow_redirects=False)

    if 'location' in dashboard.headers:

        dashboard = urlfetch.fetch(url=dashboard.headers['location'], method=urlfetch.PUT,
                                payload=json.dumps(putdata), headers=headers, follow_redirects=False)
...
def updateclientsplash(apikey, networkid, clientmac, ssid_authorization, suppressprint=False):
    ...
    if 'location' in dashboard.headers:
        ...
        #rpc.callback = __handle_result(rpc)
        urlfetch.make_fetch_call(rpc, url=dashboard.headers['location'],
                                             method=urlfetch.PUT, payload=json.dumps(putdata),
                                             headers=headers, follow_redirects=False)
        #dashboard = urlfetch.fetch(url=dashboard.headers['location'], method=urlfetch.PUT,
        #                           payload=json.dumps(putdata), headers=headers, follow_redirects=True)
    dashboard = rpc.get_result()
    result = __returnhandler(dashboard.status_code, dashboard.content, calltype, suppressprint)
...
def getdevicelist_stub(apikey, networkid, templateid=None, suppressprint=True, networkName=None):
    ...
    storeDeviceList = meraki_store.get_meraki_device_list(networkid)
    for device in result:
        ...
        deviceSerialList.append(device['serial'])

        storeDeviceID = meraki_store.get_meraki_device(device['serial'])
        try:
            dayssince = (datetime.datetime.now() - storeDeviceID.timestamp).days
            msg = 'time since last update is {} days.'.format(str(dayssince))
            logging.debug(msg)
        except:
            pass
        storeDeviceID.networkId = device['networkId']
        storeDeviceID.name = device['name']
        storeDeviceID.mac = device['mac']
        storeDeviceID.model = device['model']
        storeDeviceID.serial = device['serial']
        storeDeviceID.lanIp = device['lanIp']
        storeDeviceID.networkName = networkName
        storeDeviceID.put()
    storeDeviceList.deviceList = deviceList
    storeDeviceList.deviceSerialList = deviceSerialList
    storeDeviceList.put()
    return deviceList, deviceSerialList
...
def getnetworkdevices(apikey, networkid, suppressprint=True):
    ...
    """
    storeNetwork = meraki_store.get_meraki_network(networkid)
    storeDeviceList = meraki_store.get_meraki_device_list(networkId=networkid)


    if storeDeviceList.deviceList:
        #logging.debug(storeDeviceList.deviceSerialList)
        try:

            dayssince = (datetime.datetime.now() - storeDeviceList.timestamp).days
            logging.debug('time since last update is {} days.'.format(str(dayssince)))

            if dayssince < timeSinceMax:
                logging.info("devices for this network were refreshed less than a day ago, skipping")
                return storeDeviceList.deviceList
            else:
                logging.info("devices were retrieved over a day ago, storing again")

                deviceList, deviceSerialList = getdevicelist_stub(apikey, networkid, templateid=None,
                                                                    suppressprint=False, networkName=storeNetwork.name)


                logging.debug(deviceList)
                return deviceList
        except:
            logging.debug("in except block, couldn't find timestamp value")
            #logging.debug(storeDeviceList.deviceList)
            return storeDeviceList.deviceList


    else:
        logging.info("no existing device list found, retrieving details... for network {}".format(storeNetwork.name))
...
def getdevicedetail(apikey, networkid, serialnumber, suppressprint=False, networkName=None):
...
def getnetworklist_stub(apikey, orgid, templateid=None, suppressprint=True):
    ...
    for network in result:
        storeNetworkID = meraki_store.get_meraki_network(network['id'])
        storeNetworkID.networkId = network['id']
        storeNetworkID.name = network['name']
        storeNetworkID.orgId = network['organizationId']
        storeNetworkID.tags = network['tags']
        storeNetworkID.timeZone = network['timeZone']
        storeNetworkID.type = network['type']
        storeNetworkID.put()
        networkIDList.append(network['id'])
        networkNameList.append(network['name'])
        networkDetails = {
            'name' : network['name'],
            'id' : network['id']
        }
        networkList.append(networkDetails)

    logging.debug(networkList)
    return networkIDList, networkNameList, networkList
...
def getnetworklist(apikey, orgid, templateid=None, suppressprint=False):
    ...
    try:
        storeNetworkList = meraki_store.get_meraki_network_list(orgid)
        #logging.debug(storeNetworkList.networkIdList)
    except:
        logging.debug("Couldn't get network list as no orgId retrieved")
        return None

    if storeNetworkList.networkIdList:

        try:

            dayssince = (datetime.datetime.now() - storeNetworkList.timestamp).days
            msg = 'time since last update is {} days.'.format(str(dayssince))
            logging.debug(msg)

            if dayssince < timeSinceMax:
                logging.info("networks were refreshed less than a day ago, skipping")
                return storeNetworkList
            else:
                logging.info("networks were retrieved over a day ago, storing again")

                networkIDList, networkNameList, networkList = getnetworklist_stub(apikey,orgid,templateid=None,suppressprint=False)
                storeNetworkList.networkIdList = networkIDList
                storeNetworkList.networkNameList = networkNameList
                storeNetworkList.networkList = networkList
                #logging.debug(storeNetworkList.networkList)
                storeNetworkList.put()
                return storeNetworkList
                #logging.debug(storeNetworkList.networkList)
        ...
        logging.info("no existing network list found, retrieving details...")
...
def getnetworkdetail(apikey, networkid, suppressprint=False):
    ...
    storeNetworkID = meraki_store.get_meraki_network(networkid)

    if storeNetworkID.networkId == "":
        ...
        storeNetworkID.networkId= networkid
        storeNetworkID.put()
        logging.debug("existing networkId not found, stored a new one")
        return storeNetworkID
    else:
        logging.debug("found existing networkId, returning this")
        return storeNetworkID
...
def myorgaccess(apikey, suppressprint=False):
    ...
    storeOrgID = meraki_store.get_meraki_organisations(apikey)

    if not storeOrgID.orgId:
        calltype = 'Organization'
        geturl = '{0}/organizations'.format(str(base_url))
        headers = {
            'X-Cisco-Meraki-API-Key': format(str(apikey)),
        ...
        #dashboard = urlfetch.fetch(geturl, headers=headers)
        dashboard = urlfetch.fetch(url=geturl,headers=headers)
        #
        # Call return handler function to parse Dashboard response
        #
        result = __returnhandler(dashboard.status_code, dashboard.content, calltype, suppressprint)
        logging.debug("result from myorgaccess is: {}".format(result))
        orgID = str(result[0]['id'])
        logging.debug("orgId determined as {}".format(orgID))
        ### Store Meraki organisation ID #
        storeOrgID.orgId = orgID
        storeOrgID.put()
        logging.debug("existing orgID not found, stored a new one")
        return orgID

    else:
        logging.debug("found existing orgId: {}".format(storeOrgID.orgId))
        return storeOrgID.orgId
...
def appendnonmerakivpn(apikey, orgid, names, ips, secrets, remotenets, tags=None, suppressprint=False):
    ...
    currentpeers = json.loads(urlfetch.fetch(geturl, headers=headers).text)
...
'''def updatessidobject(apikey, networkid, newssid: SSID, suppressprint=False):
    ...
    # Call return handler function to parse Dashboard response
    #
    result = __returnhandler(dashboard.status_code, dashboard.content, str(newssid.type).upper(), suppressprint)
    return result
'''
